...
from federated_learning.lib.core.helpers import Namespace, SuperFactory
...
from federated_learning.lib.model.layers import GraphConvolutionWrapper, LinearBlock, TripletMessagePassingLayer
...
class AbstractNetwork(torch.nn.Module, metaclass=ABCMeta):
    ...
    def load_checkpoint(self, checkpoint_path: str, device: Optional=None):
...
class EnsembleNetwork(AbstractNetwork):
    ...
    def load_checkpoint(self, checkpoint_paths: List[str], device: Optional=None):
...
class SimpleMLPNetwork(AbstractNetwork):
    """ Multi layers perceptron Network.
    ...
    def __init__(
        ...
        for i in range(n_layer - 1):
            layers.append(torch.nn.Linear(**args))
            layers.append(torch.nn.ReLU())
            layers.append(torch.nn.Dropout(p=dropout))

            args["in_features"] = args["out_features"]
            args["out_features"] = hidden_features
