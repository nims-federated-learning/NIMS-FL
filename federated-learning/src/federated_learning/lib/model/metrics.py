...
class AvailableMetrics:
    MAE = MetricConfiguration(
        type=MetricType.REGRESSION, calculator=mean_absolute_error, maximize=False
    )
    MSE = MetricConfiguration(
        type=MetricType.REGRESSION, calculator=mean_squared_error, maximize=False
    )
    RMSE = MetricConfiguration(
        type=MetricType.REGRESSION,
        calculator=partial(mean_squared_error, squared=False),
        maximize=False,
    )
    R2 = MetricConfiguration(type=MetricType.REGRESSION, calculator=r2_score)
    PEARSON = MetricConfiguration(
        type=MetricType.REGRESSION,
        calculator=CustomMetrics.pearson_correlation_coefficient,
    )
    SPEARMAN = MetricConfiguration(
        type=MetricType.REGRESSION,
        calculator=CustomMetrics.spearman_correlation_coefficient,
    )
    KL_DIV = MetricConfiguration(
        type=MetricType.REGRESSION,
        calculator=CustomMetrics.kullback_leibler_divergence,
        maximize=False,
    )
    JS_DIV = MetricConfiguration(
        type=MetricType.REGRESSION,
        calculator=CustomMetrics.jensen_shannon_divergence,
        maximize=False,
    )
    CHEBYSHEV = MetricConfiguration(
        type=MetricType.REGRESSION, calculator=distance.chebyshev, maximize=False
    )
    MANHATTAN = MetricConfiguration(
        type=MetricType.REGRESSION, calculator=distance.cityblock, maximize=False
    )
    RANK_QUALITY = MetricConfiguration(
        type=MetricType.REGRESSION, calculator=CustomMetrics.rank_quality
    )

    ROC_AUC = MetricConfiguration(
        type=MetricType.CLASSIFICATION, calculator=roc_auc_score
    )
    PR_AUC = MetricConfiguration(
        type=MetricType.CLASSIFICATION, calculator=average_precision_score
    )
    ACCURACY = MetricConfiguration(
        type=MetricType.CLASSIFICATION, calculator=accuracy_score, uses_threshold=True
    )
    PRECISION = MetricConfiguration(
        type=MetricType.CLASSIFICATION,
        calculator=partial(precision_score, zero_division=1),
        uses_threshold=True,
    )
    RECALL = MetricConfiguration(
        type=MetricType.CLASSIFICATION,
        calculator=partial(recall_score, zero_division=1),
        uses_threshold=True,
    )
    F1 = MetricConfiguration(
        type=MetricType.CLASSIFICATION, calculator=f1_score, uses_threshold=True
    )
    COHEN_KAPPA = MetricConfiguration(
        type=MetricType.CLASSIFICATION,
        calculator=cohen_kappa_score,
        uses_threshold=True,
    )
    JACCARD = MetricConfiguration(
        type=MetricType.CLASSIFICATION, calculator=jaccard_score, uses_threshold=True
    )
